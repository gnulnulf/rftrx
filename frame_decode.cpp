...
int protoguess[ PROTO_COUNT ];

int decode_1331( String &sFrame ) {
  int framelen = sFrame.length() ;
  int decoded=0;
/*      inverted 1 2 3 4 5  A B C D E ON
A >DATA1331:23:1010101010 0010101010 0010
AOFF                                OFF
  >DATA1331:23:0000000000 001010101 0100
Arelease?                           RELEASE
 ...
 */
 ...
 if ( (framelen == 23) or  (framelen == 24)  ) {
  ...
  String data= "z-z-z-z-z-a-a-a-a-a-x-x";
                Serial.print("IDX=");
                Serial.print( getbinfromframe( 'z', sFrame, data,0));
                Serial.print(";SWITCH=");
                Serial.print( getbinfromframe( 'a', sFrame, data,0));

                Serial.print(";ACTION=");
                Serial.print( getbinfromframe( 'x', sFrame, data,0));
                Serial.print(";frame=");
                Serial.print( sFrame);
                Serial.println();
    decoded=1;
...
int frame_decode(String &frame) {
  ...
  // mark all protocols 0%
  for (int i=0;i<PROTO_COUNT;i++) protoguess[i]=0;
